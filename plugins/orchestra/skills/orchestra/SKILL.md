...
다음 레퍼런스를 Read로 읽는다:
- `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/orchestra/references/brief-interview.md`
- `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/orchestra/references/model-recommendation.md`
- `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/orchestra/references/plan-prompt.md`
- `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/orchestra/references/reviewer-prompt.md`

브라우저 자동화 첫 호출 직전에 읽는다:
- `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/orchestra/references/chatgpt-selectors.md`
...
```bash
bash ${HOME}/.claude/skills/orchestra/scripts/orchestra-init.sh "<slug>"
```
...
2. reviewer-bridge로 호출:
   ```bash
   bash $HOME/.claude/skills/orchestra/scripts/reviewer-bridge.sh \
     --model $(yq '.stage2_model' $RUN_DIR/01-brief.md) \
     "$RUN_DIR/_tmp/plan-prompt.md" \
...
2. reviewer-bridge로 호출:
   ```bash
   bash $HOME/.claude/skills/orchestra/scripts/reviewer-bridge.sh \
     --model $(yq '.stage5_model' $RUN_DIR/01-brief.md) \
     "$RUN_DIR/_tmp/review-prompt-v<round>.md" \
...
```bash
bash $HOME/.claude/skills/orchestra/scripts/score-check.sh "$RUN_DIR/05-review-v<round>.md"
```
